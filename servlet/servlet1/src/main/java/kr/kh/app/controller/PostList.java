...
public class PostList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			CommunityVO communityInfo = postService.getcommunityInfo(coNum);
			...
			//서비스에게 커뮤니티 번호를 주면서 게시글 리스트를 가져오라고 시킴 // ++ cri.co_num 으로 가져올 수 있게 됨 
			List<PostVO> list = postService.getPostList(cri);
			
			//화면에 커뮤니티 정보를 전송
			request.setAttribute("communityInfo", communityInfo);
			//화면에 가져온 게시글 리스트를 전송 
