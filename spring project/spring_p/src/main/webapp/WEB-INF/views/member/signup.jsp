...
<body>
	...
	<form action="<c:url value="/signup"/>" method="post" id="form">
		...
		<div class="form-group">
			<label for="pw2">비번 확인:</label> <input type="password"
	...
	<script type="text/javascript">
		...
		*/
		function checkId(id){
			...
			//맞으면 서버에 확인 요청
			$.ajax({
				async : false, //비동기 // true(비동기) : 서버로 데이터가 전송되는 시간 안 기다림, false(동기) : 시간 걸려도 기다려줌
		...
		function displayCheckId(result){
			...
			if(result == 1){
				...
				var str = `<label id="check-id" class="error">이미 사용 중인 아이디입니다.</label>`;
				$('#id').after(str);
			}
		}
	</script>
