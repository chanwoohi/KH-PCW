...
public class StudentManager implements Program{
	...
	private void studentInfoUpdate() {
		...
		//생성한 학생 객체를 이용해서 리스트에 몇 번지에 있는지 번지를 가져옴
		int index = 0 ;
		...
		if(index < 0) {
			System.out.println("유효하지 않은 번지 입니다.");
		}
	...
	private void insertSubjectScore() {
		...
		Student std = inputStudent();
		//리스트에 입력한 학생 객체가 몇 번지에 있는지 번지를 가져옴
		int index = list.indexOf(std);
	...
	private void updateSubjectScore() {
		...
		//학생 정보를 입력하여 객체를 생성
		Student std = inputStudent();
		//학생 리스트에서 학생 정보가 몇 번지에 있는지 번지를 가져옴
		int index = list.indexOf(std);
		...
		//subjects에서 해당 과목이 몇 번지에 있는지 확인해서 해당 과목 정보를 가져오고
		...
		//과목이 과목리스트에 없으면 안내문구 출력 후 종료
		if(!subjectList.contains(subject.getName())) {
			System.out.println("등록되지 않은 과목이어서 수정할 수 없습니다.");
			return;
		}

		//과목 객체가 학생 성적 리스트에 없으면 안내문구 출력 후 종료
		if(!subjects.contains(subject)) {
			System.out.println("등록되지 않은 과목 성적이어서 수정할 수 없습니다.");
			return;
		}
		//중간, 기말, 수행평가를 입력
		System.out.print("중간 : ");
	...
	private void deleteSubjectScore() {
		...
		//과목, 학년, 학기를 이용해서 객체를 생성
		Subject tmp = new Subject(name, grade, semester);
		//학생 과목 리스트에서 과목 객체를 삭제하여 성공하면 안내문구 출력 후 종료
		if(tmpList.remove(tmp)) {
	...
	private void defaultPrint() {
		System.out.println("올바른 메뉴를 선택 하세요.");
	}
	...
	private void insertSubject() {
		...
		//과목 리스트에 등록된 과목인지 확인해서 등록되어있으면 안내문구 출력 후 종료
	...
	private void updateSubject() {
		...
		scan.nextLine();
		String subject = scan.nextLine();
		//수정할 과목이 있는지 없는지 확인해서 없으면 알림 문구 출력 후 종료
		if(!subjectList.contains(subject)) {
