...
# 카테고리별 제품의 평균 가격이 3만원 이상인 카테고리
# avg 같은 직계함수는 where절이 아니라 having 절을 사용해야하고 위치는 group by 뒤
select ca_name, format(ifnull(avg(pr_price), 0),0) '평균가격' from category
left join product on pr_ca_num = ca_num
group by ca_num
...
# 카테고리별 등록된 제품수를 조회
select ca_name, count(pr_name) 수량 from category
left join product on pr_ca_num = ca_num
group by ca_num;
...
insert into basket(ba_pr_code, ba_me_id, ba_amount) values('ACC001', 'abc123', '3');

# 'abc123' 회원이 장바구니에 1번 제품 2개를 담았을 때 쿼리
update basket set ba_amount = 2 where ba_pr_code = 'ACC001' and ba_me_id = 'abc123';

# 'abc123' 회원이 장바구니에 있는 `ACC001` 제품을 2개 구매했을 때 쿼리
insert into buy(bu_pr_code, bu_me_id, bu_amount, bu_state, bu_date)
values('ACCOO1', 'abc123', 2, '구매', now());
delete from basket where ba_me_id = 'abc123' and bu_pr_code = 'ACC001';